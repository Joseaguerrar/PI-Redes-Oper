...
## Paso 2: Ejecutar el cliente NachOS (nachitos)
...
### 2.3. Ejecutar el cliente nachitos

```bash
./nachos -x ../test/nachitos
```

Este comando hace que NachOS cargue el programa nachitos, que se conecta al servidor por socket, envía la petición HTTP y muestra la respuesta (por ejemplo, una figura en ASCII).

Resultado esperado
El servidor mostrará en consola que recibió la petición.

El cliente nachitos imprimirá la figura recibida en la terminal.
