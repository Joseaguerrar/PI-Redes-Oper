...
int VSocket::EstablishConnection( const char * hostip, int port ) {
  ...
  if (this->IPv6) {
   ...
   // Convertir IP y verificar errores
   ...
   // Establecer interfaz (scope_id) si es una dirección link-local
   server_addr6.sin6_scope_id = if_nametoindex("eth0"); // o la interfaz correcta en tu sistema
   if (server_addr6.sin6_scope_id == 0) {
       perror("if_nametoindex error");
       throw std::runtime_error("VSocket::EstablishConnection(), error en scope_id");
   }

   // Conectar
   if (connect(this->idSocket, (struct sockaddr*)&server_addr6, sizeof(server_addr6)) == -1) {
       perror("connect error");
       throw std::runtime_error("VSocket::EstablishConnection(), error al conectar IPv6");
   }
} else {
      ...
      if (inet_pton(AF_INET, hostip, &server_addr4.sin_addr) <= 0) {
          perror("inet_pton error");
          throw std::runtime_error("VSocket::EstablishConnection(), error en inet_pton IPv4");
      }
  
      if (connect(this->idSocket, (struct sockaddr*)&server_addr4, sizeof(server_addr4)) == -1) {
          perror("connect error");
          throw std::runtime_error("VSocket::EstablishConnection(), error al conectar IPv4");
      }
  }
