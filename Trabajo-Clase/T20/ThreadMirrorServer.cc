...
// Estructura para representar a un servidor
struct ServerInfo
{
   std::string nombre;
   std::string ip;
...
std::map<std::string, ServerInfo> tabla_figuras; // figura -> ServerInfo
...
{
   const int BROADCAST_WAIT = 120;
   const int TIMEOUT_RESPONSE = 5;
   const int DISCOVERY_PORT = 5353;
   std::vector<std::string> broadcast_ips = {
       "172.16.123.15", "172.16.123.31", "172.16.123.47",
       "172.16.123.63", "172.16.123.79", "172.16.123.95", "172.16.123.111"};
   ...
   while (true)
   {
      std::string mensaje = "GET /servers";
      ...
      // Esperar respuestas por 5 segundos
      ...
      while (true)
      {
         ...
         if (std::chrono::duration_cast<std::chrono::seconds>(now - start).count() > TIMEOUT_RESPONSE)
            break;
         ...
         std::istringstream iss(respuesta);
         std::string nombre, ip, lista;

         if (std::getline(iss, nombre, '|') &&
             std::getline(iss, ip, '|') &&
             std::getline(iss, lista))
         {
            ...
            std::istringstream figuras_stream(lista);
            std::string figura;
            while (std::getline(figuras_stream, figura, ','))
            {
               info.figuras.push_back(figura);
            }

            std::lock_guard<std::mutex> lock(tabla_mutex);
            for (const auto &fig : info.figuras)
            {
               tabla_figuras[fig] = info;
            }
         }
      }

      std::this_thread::sleep_for(std::chrono::seconds(BROADCAST_WAIT));
   }
}
...
int main(int argc, char **argv) {
   std::thread discovery_thread(server_discovery);
   discovery_thread.detach(); // No bloquea
